<template>
    ...
    <div v-if="!appStore.isMobile">
        <v-dialog v-model="appStore.dlgWithdrawDsa" max-width="647" style="z-index: 2;">
            <v-card color="black" class="clsDialogWithdrawDsa">
                <v-toolbar dark color="black">
                    <v-spacer></v-spacer>
                    <v-btn icon variant="plain" dark @click="appStore.dlgWithdrawDsa = false">
                        <v-icon>close</v-icon>
                    </v-btn>
                </v-toolbar>
                ...
                <v-card-title :class="appStore.isMobile ? 'd-flex align-center mx-auto' : 'd-flex align-center'"
                    ...
                    <v-btn icon variant="plain" :style="appStore.isMobile ? '' : 'margin-left: 25px;'"
                        @click="appStore.dlgWithdrawDsa = false">
                        <BackButton />
                    </v-btn>
                    <p class="font-bold text-white text-weight-700 text-size-22px">Withdraw ( DSA )</p>
                </v-card-title>
                        ...
                        <v-container class="pt-0 pb-0">
                            <v-row no-gutters class="mt-5 rowWithdraw">
                                ...
                                <v-col align-self="center" cols="5">
                                    <p class="font-regular text-white text-weight-700 text-size-18px mr-5"
                                        ...
                                        {{ parseFloat(appStore.dsa_usdc_balance.toFixed(6)) }} USDC</p>
                                </v-col>
                                <v-col align-self="center" cols="3">
                                    <!-- <withdraw-input withdraw_token="usdc" /> -->
                                    <withdraw-dsa-input withdraw_token="usdc" />
                                </v-col>
                            </v-row>
                            <v-row no-gutters class="mt-8 rowWithdraw">
                                ...
                                <v-col align-self="center" cols="3">
                                    ...
                                    <withdraw-dsa-input withdraw_token="eth" />
                                </v-col>
                            </v-row>
                            <v-row no-gutters class="mt-8 rowWithdraw">
                                <v-col align-self="center" cols="2">
                                ...
                                <v-col align-self="center" cols="5">
                                    <p class="font-regular text-white text-weight-700 text-size-18px mr-5"
                                        style="text-align: end;">
                                        {{ parseFloat(appStore.dsa_wbtc_balance.toFixed(8)) }} BTC</p>
                                </v-col>
                                <v-col align-self="center" cols="3">
                                    <!-- <withdraw-input withdraw_token="btc" /> -->
                                    <withdraw-dsa-input withdraw_token="btc" />
                                </v-col>
                            </v-row>
    ...
    <div v-else>
        <v-dialog v-model="appStore.dlgWithdrawDsa" fullscreen transition="fade-transition" style="z-index: 2;">
            <v-card color="black" class="clsDialogWithdrawDsaMobile">
                <layout-header />
                <v-card-title class="d-flex align-center mx-auto" style="margin-top: 43px;">
                    <v-btn icon variant="plain" @click="appStore.dlgWithdrawDsa = false">
                        <BackButton />
                    </v-btn>
...
import WithdrawDsaInput from './WithdrawDsaInput.vue'
