<template>
    <div>
        <v-dialog v-model="dialogBorrowConfig" :max-width="appStore.isMobile ? 5000 : 647"
            ...
            <template v-slot:activator="{ props }">
                ...
                <v-btn v-if="!appStore.isMobile" v-bind="props"
                    class="btnConfirmBorrow text-none font-regular text-white text-weight-700 text-size-16px"
                    :disabled="props_info.btn_disable">Confirm Borrow</v-btn>
                <!-- Borrow Confirm Button - Mobile -->
                <v-btn v-else v-bind="props"
                    class="btnConfirmBorrowMobile text-none font-regular text-white text-weight-700 text-size-15px"
                    :disabled="props_info.btn_disable">Confirm Borrow</v-btn>
            </template>
            ...
            <v-card color="black" :class="appStore.isMobile ? '' : 'clsDialog'"
                ...
                <v-toolbar v-if="!appStore.isMobile" dark color="black">
                    <v-spacer></v-spacer>
                    <v-btn icon variant="plain" dark @click="dialogBorrowConfig = false">
                        <v-icon>close</v-icon>
                    </v-btn>
                </v-toolbar>
                <!-- Toolbar - Mobile -->
                ...
                <v-card-title :class="appStore.isMobile ? 'd-flex align-center mx-auto' : 'd-flex align-center'"
                    ...
                    <v-btn icon variant="plain" :style="appStore.isMobile ? '' : 'margin-left: 25px;'"
                        @click="dialogBorrowConfig = false">
                        <BackButton />
                    </v-btn>
                ...
                <v-card-text class="ma-0 pa-0">
                    <div style="text-align: center;">
                        ...
                        <v-container :class="appStore.isMobile ? 'containerBorrowConfirmMobile' : 'containerBorrowConfirm'">
                            <!-- Selected Token -->
                            ...
                            <v-row class="mt-5">
                                <v-col cols="7" align-self="center" style="text-align: start;">
                                    <p class="font-regular text-white text-weight-400 text-size-18px">Borrowed Amount</p>
                                </v-col>
                                <v-col cols="5" style="text-align: start;" align-self="center">
                                    <p class="font-regular text-white text-weight-700 text-size-18px">
                                        {{ props_info.borrow_amount }} {{ props_info.selected_token.toUpperCase() }}
                                    </p>
                                </v-col>
                            </v-row>
                            <v-row class="mt-5">
                            ...
                            <v-row no-gutters style="margin-top: 30px;">
                                <v-dialog v-model="dialogBorrowResult" :max-width="appStore.isMobile ? 5000 : 647"
                                    :fullscreen="appStore.isMobile ? true : false"
                                    ...
                                    <v-card color="black" :class="appStore.isMobile ? '' : 'clsDialog'"
                                        ...
                                        <v-toolbar v-if="!appStore.isMobile" dark color="black">
                                            <v-spacer></v-spacer>
                                            <v-btn icon variant="plain" dark @click="closeDialogBorrow"
                                                :disabled="!actionBorrowDone">
                                                <v-icon>close</v-icon>
                                            </v-btn>
                                        </v-toolbar>
                                        <!-- Toolbar - Mobile -->
                                        ...
                                        <v-card-text class="ma-0 pa-0">
                                            <div style="text-align: center;">
                                                <v-container
                                                    :class="appStore.isMobile ? 'containerBorrowConfirmMobile' : 'containerBorrowConfirm'">
                                                    ...
                                                    <v-row no-gutters style="margin-top: 35px; margin-bottom: 215px;">
                                                        <v-btn variant="plain"
                                                            :class="appStore.isMobile ? 'btnConfirmBorrowMobile text-none font-regular text-white text-weight-700 text-size-15px' : 'btnConfirmBorrow text-none font-regular text-white text-weight-700 text-size-16px'"
                                                            @click="closeDialogBorrow" :disabled="!actionBorrowDone">
                                                            Back
                                                        </v-btn>
                                                    </v-row>
                                                </v-container>
                            ...
                            <v-row no-gutters
                                :style="appStore.isMobile ? 'margin-top: 125px;' : 'margin-top: 145px; margin-bottom: 30px;'">
                                <v-spacer></v-spacer>
...
import BorrowResultBanner from "../assets/borrowRes.png";
...
const dialogBorrowConfig = ref(false);
const dialogBorrowResult = ref(false);

function closeDialogBorrow() {
    dialogBorrowConfig.value = false;
    dialogBorrowResult.value = false;
}

const actionBorrowDone = ref(false);
const borrow_success = ref(false);
const result_text = ref("");
async function actionBorrow() {
    appStore.dlgSwapEnable = false;
    actionBorrowDone.value = false;
    borrow_success.value = false;
    try {
        //------- Start Borrow ---------
        let tokenAddr = "";
        let amount = "";
        if (props_info.selected_token == "usdc") {
            ...
            amount = BigNumber(props_info.borrow_amount).multipliedBy(1e6).toString();
        } else if (props_info.selected_token == "eth") {
            tokenAddr = appStore.WETH;
            amount = BigNumber(props_info.borrow_amount).multipliedBy(1e18).toString();
        } else if (props_info.selected_token == "btc") {
            tokenAddr = appStore.WBTC;
            amount = BigNumber(props_info.borrow_amount).multipliedBy(1e8).toString();
        }
        ...
        // Borrow Token
        doughV1DsaContract.methods.doughCall(1, 2, tokenAddr, tokenAddr, amount, false).send({ from: address.value })
            .on('confirmation', async function (confirmationNumber: any, receipt: any) {
                if (confirmationNumber == appStore.confirmLimit) {
                    ...
                    result_text.value = "You have successfully borrowed!";
                    borrow_success.value = true;
                    console.log("Borrow Token receipt: ", receipt);
                    appStore.dlgSwapEnable = true;
                    actionBorrowDone.value = true;
                }
            })
            .on('error', function (error: any) {
                result_text.value = "Failed!";
                borrow_success.value = true;
                console.log("Borrow Token error: ", error);
                appStore.dlgSwapEnable = true;
                actionBorrowDone.value = true;
            });
        //------- End Borrow -----------
