        ...
        <v-dialog v-model="dialogShield" :max-width="appStore.isMobile ? 5000 : 647"
            ...
            <template v-slot:activator="{ props }">
                ...
                <v-btn v-if="!appStore.isMobile" v-bind="props"
                    class="btnConfirmShield text-none font-regular text-white text-weight-700 text-size-16px">Stop</v-btn>
                <!-- Shield Confirm Button - Mobile -->
                <v-btn v-else v-bind="props"
                    class="btnConfirmShieldMobile text-none font-regular text-white text-weight-700 text-size-15px">Stop</v-btn>
            </template>
...
async function actionShield() {
    ...
    try {
        ...
        // stop shield
        doughV1DsaIndex.methods.unsetShield().send({ from: address.value })
            .on('confirmation', async function (confirmationNumber: any, receipt: any) {
                if (confirmationNumber == appStore.confirmLimit) {
                    ...
                    console.log("shield Stop receipt: ", receipt);
                }
            })
            .on('error', function (error: any) {
                ...
                actionShieldDone.value = true;
                appStore.dlgSwapEnable = true;
                console.log("shield Stop error: ", error);
            });
        //------- End Stop Shield -----------
    } catch (error) {
        closeDialogShield();
