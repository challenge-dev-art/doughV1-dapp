<template>
  <div>
    <v-dialog v-model="dialogWithdrawDsa" :max-width="appStore.isMobile ? 5000 : 647"
        ...
        <v-sheet v-else v-bind="props" class="d-flex align-center btnWithdrawMobile" @click="initInputInfo">
          ...
          <div>
            <p class="font-regular text-white text-weight-400 text-size-18px">
              {{ props_token.withdraw_token == 'usdc' ? parseFloat(appStore.dsa_usdc_balance.toFixed(6))
                : props_token.withdraw_token == 'eth' ? parseFloat(appStore.dsa_weth_balance.toFixed(6))
                  : props_token.withdraw_token == 'btc' ? parseFloat(appStore.dsa_wbtc_balance.toFixed(6)) : 0
      ...
      <v-card color="black" :class="appStore.isMobile ? '' : 'clsDialog'"
        ...
        <v-toolbar v-if="!appStore.isMobile" dark color="black">
          <v-spacer></v-spacer>
          <v-btn icon variant="plain" dark @click="dialogWithdrawDsa = false">
            <v-icon>close</v-icon>
          </v-btn>
        </v-toolbar>
        <!-- Toolbar - Mobile -->
        ...
        <v-card-title :class="appStore.isMobile ? 'd-flex align-center mx-auto' : 'd-flex align-center'"
          ...
          <v-btn icon variant="plain" :style="appStore.isMobile ? '' : 'margin-left: 25px;'"
            @click="dialogWithdrawDsa = false">
            <BackButton />
          </v-btn>
          <p class="font-bold text-white text-weight-700 text-size-22px">
            Withdraw {{ selectedToken.toUpperCase() }} ( DSA )</p>
        </v-card-title>
            ...
            <v-container :class="appStore.isMobile ? 'containerWithdrawInputMobile' : 'containerWithdrawInput'">
              ...
              <v-row no-gutters :class="appStore.isMobile ? 'amountTokenMobile' : 'amountToken'">
                <v-col align-self="center" style="text-align: start;">
                  <input v-model="amountWithdraw" type="number" @keypress="enterDetect" id="idWithdrawDsa"
                    name="nameWithdrawDsa" placeholder="Insert amount to withdraw" bg-color="#00C797" color="#FFFFFF"
              ...
              <div no-gutters
                ...
                <!-- <v-text-field v-model="address" variant="plain" placeholder="Enter address" readonly bg-color="#1E1E1E"
                  color="#FFFFFF" hide-details class="font-light text-white text-weight-400 text-size-16px">
                </v-text-field> -->
                {{ appStore.isMobile ? shortenAddress(address) : address }}&nbsp;&nbsp;&nbsp;
                <copy-button v-if="address != ''" :address="address" />
                <!-- <v-btn icon="content_copy" variant="plain" @click="addressWithdraw = address"
                  style="margin-top: 15px;"></v-btn> -->
              </div>

              <!-- Deposit Confirm Button -->
              <v-row no-gutters :style="appStore.isMobile ? 'margin-top: 25px;' : 'margin-top: 33px;'">
                <withdraw-dsa-confirm :selected_token="selectedToken"
                  :withdraw_amount="isNaN(parseFloat(amountWithdraw)) ? 0 : parseFloat(amountWithdraw)"
                  :to_address="address" :gas_fee="gasFee" :gas_fee_usd="gasFeeUsd" :btn_disable="btn_disabled"
                  style="width: 100%" />
              </v-row>
...
import WithdrawDsaConfirm from "./WithdrawDsaConfirm.vue";
...
function initInputInfo() {
  ...
  // addressWithdraw.value = "";
}

const btnDisable = computed(() => {
  switch (props_token.withdraw_token) {
    case "usdc":
      return appStore.dsa_usdc_balance == 0 ? true : false;
    case "eth":
      return appStore.dsa_weth_balance == 0 ? true : false;
    case "btc":
      return appStore.dsa_wbtc_balance == 0 ? true : false;
    default:
      return true;
  }
});
...
// const addressWithdrawValid = ref(false);
// watch(addressWithdraw, (newVal) => {
//   const web3 = new Web3(wallet.provider as any);
//   if (web3.utils.isAddress(newVal)) {
//     addressWithdrawValid.value = true;
//   } else {
//     addressWithdrawValid.value = false;
//   }
// });

const btn_disabled = computed(() => {
  // return !amountWithdrawValid.value || !addressWithdrawValid.value;
  return !amountWithdrawValid.value;
});
watch(btn_disabled, () => {
  getGasFee();
});
function setMax() {
  if (selectedToken.value == "usdc") {
    amountWithdraw.value = appStore.dsa_usdc_balance.toString();
  } else if (selectedToken.value == "eth") {
    amountWithdraw.value = appStore.dsa_weth_balance.toString();
  } else if (selectedToken.value == "btc") {
    amountWithdraw.value = appStore.dsa_wbtc_balance.toString();
  } else {
    amountWithdraw.value = "";
  }
}
...
async function getGasFee() {
  if (
    ...
    // withdraw Token
    doughV1DsaContract.methods.doughCall(0, 3, tokenAddr, tokenAddr, amount, false).estimateGas({ from: address.value })
      .then(async (gas: any) => {
        // console.log("gas: ", gas)
