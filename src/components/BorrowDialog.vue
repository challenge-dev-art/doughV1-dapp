<template>
    <!-- For Desktop & Mobile -->
    <div>
        <v-dialog v-model="appStore.dlgBorrow" :max-width="appStore.isMobile ? 5000 : 647"
            :fullscreen="appStore.isMobile ? true : false"
            :transition="appStore.isMobile ? 'fade-transition' : 'dialog-transition'" style="z-index: 2;">
            <v-card color="black" :class="appStore.isMobile ? 'clsDialogBorrowMobile' : 'clsDialogBorrow'">
                ...
                <v-toolbar v-else dark color="black">
                    <v-spacer></v-spacer>
                    <v-btn icon variant="plain" dark @click="appStore.dlgBorrow = false">
                        <v-icon>close</v-icon>
                    </v-btn>
                </v-toolbar>
                ...
                <v-card-title v-if="appStore.isMobile" class="d-flex align-center mx-auto" style="margin-top: 43px;">
                    <v-btn icon variant="plain" @click="appStore.dlgBorrow = false">
                        <BackButton />
                    </v-btn>
                ...
                <v-card-title v-else>
                    <v-row no-gutters>
                        <v-col class="d-flex pa-0" align-self="center">
                            <v-btn icon variant="plain" style="margin-left: 53px;" @click="appStore.dlgBorrow = false">
                                <BackButton />
                            </v-btn>
                        ...
                        <v-col style="text-align: end;" align-self="center">
                            <v-btn class="text-none font-regular text-white text-weight-700 text-size-17px"
                ...
                <v-card-text class="ma-0 pa-0">
                    <div style="text-align: center;">
                        <v-container :class="appStore.isMobile ? 'containerBorrowDialogMobile' : 'containerBorrowDialog'">
                            ...
                            <v-row no-gutters style="margin-top: 30px;">
                                <v-col>
                                    <p
                                        ...
                                        Select which token you would like to borrow.<br />
                                        You must have funds in order to borrow.</p>
                                </v-col>
                            </v-row>

                            <div v-if="appStore.isMobile">
                                <v-container class="pa-0">
                                    <borrow-input borrow_token="usdc" />
                                    <borrow-input borrow_token="eth" />
                                    <borrow-input borrow_token="btc" />
                                </v-container>
                            </div>
                            <div v-else>
                                <v-row no-gutters class="rowDepositBorrowDlg" style="margin-top: 10px;">
                                ...
                                <v-row no-gutters class="rowDepositBorrowDlg" style="margin-top: 15px;">
                                    <v-col align-self="center" cols="2">
                            ...
                            <v-row no-gutters
                                :style="appStore.isMobile ? 'margin-top: 170px;' : 'margin-top: 175px; margin-bottom: 30px;'">
                                <v-spacer></v-spacer>
...
import BorrowInput from "./BorrowInput.vue"
