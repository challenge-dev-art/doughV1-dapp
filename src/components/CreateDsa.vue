<template>
    <div>
        <v-dialog v-model="appStore.dlgCreateDsa" :max-width="appStore.isMobile ? 5000 : 647"
            ...
            <v-card color="black" :class="appStore.isMobile ? '' : 'clsDialog'"
                ...
                <v-toolbar dark color="black">
                    <v-spacer></v-spacer>
                    <v-btn icon variant="plain" dark @click="appStore.dlgCreateDsa = false">
                        <v-icon>close</v-icon>
                    </v-btn>
                </v-toolbar>
                    ...
                    <div style="text-align: center;">
                        <v-container :class="appStore.isMobile ? 'containerConnectWalletMobile' : 'containerConnectWallet'">
                            ...
                            <p
                                ...
                                Error! You have not created your DSA
                            </p>
                            <div style="margin-top: 35px; margin-bottom: 150px;">
                                <div>
                                    <v-btn variant="tonal" @click="gotoCreateDsa" class="text-none btnConnectWallet">
                                        Create DSA</v-btn>
                                </div>
                            </div>
                        </v-container>
...
function gotoCreateDsa() {
    appStore.dlgCreateDsa = false;
    appStore.dlgGetStartedTab = "two";
    appStore.dlgGetStarted = true;
}
